...
const MenuItem: FC<MenuItemProps> = ({ icon: Icon, text, subtext, date, type, onClick }) => (
  <motion.div
    className="flex items-center justify-between py-2 px-2 cursor-pointer font-['SF_Pro_Display']" 
    onClick={onClick}
    whileHover={{
      scale: 0.95,
  ...
  >
    ...
    <div className="flex items-center">
      ...
      {date && <div className="text-xs font-['SF_Pro_Display'] text-[#634935]">{date}</div>}
    </div>
  </motion.div>
);
...
const MenuBar: FC = () => {
  ...
  const menuItems: Record<string, MenuItemProps[]> = {
    Apps: [
      {
        icon: () => <div className="w-6 h-6 bg-gray-800 text-white flex items-center font-['SF_Pro_Display'] justify-center rounded mr-2">K</div>,
        text: "Klack",
  ...
  return (
    ...
    <div className=" bottom-24 left-0 right-0 p-2 font-['SF_Pro_Display']">
      <motion.div 
        ref={menuRef} 
